...
import { Edit, Trash2, Plus, AlertCircle, Calendar } from 'lucide-react';
...
const EventList = () => {
  ...
  const handleDelete = async () => {
    if (itemToDelete) {
      ...
      const newTotalPages = Math.ceil((events.length - 1) / ITEMS_PER_PAGE);
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Kelola Acara / Event</h1>
          <p className="text-gray-600 mt-1">Daftar semua acara yang akan dan telah diselenggarakan.</p>
        </div>
        <Link to="/admin/event/tambah" className="btn-primary flex items-center gap-2">
          <Plus size={20} />
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-100 text-gray-500 text-sm">
                <th className="p-4 font-medium">Judul Acara</th>
                <th className="p-4 font-medium">Lokasi</th>
                <th className="p-4 font-medium">Waktu Pelaksanaan</th>
              ...
              {events.length === 0 ? (
                <tr>
                  <td colSpan="4" className="p-8 text-center text-gray-500">
                    Belum ada acara. Silakan klik "Tambah Acara".
                  </td>
                </tr>
              ) : (
                currentItems.map((item) => (
                  <tr key={item.id} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                    ...
                    <td className="p-4 text-gray-600 text-sm">
                      <div className="font-medium">{item.startDate} {item.startTime && `(${item.startTime})`}</div>
                      <div className="text-gray-400 text-xs mt-1">s/d {item.endDate} {item.endTime && `(${item.endTime})`}</div>
                    </td>
                    <td className="p-4 flex items-center justify-end gap-2">
                      <Link to={`/admin/event/edit/${item.id}`} className="px-3 py-1.5 flex items-center gap-1.5 text-blue-700 bg-blue-50 hover:bg-blue-100 rounded-lg transition-colors text-sm font-medium">
                        <Edit size={16} />
                        <span>Edit</span>
                      </Link>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-xl">
            ...
            <p className="text-gray-600 mb-6">
              Apakah Anda yakin ingin menghapus acara <strong>{itemToDelete?.title}</strong>?
            </p>
